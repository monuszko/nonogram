...
class Board:
    ...
    def solve(self, no_hints=False, legend='*.@', 
        ...
        while progress:
            ...
            if self.issolved(): # To avoid the single unneeded guess
                break
            if not hide_progress:
        ...
        for y in range(self.height):
            for x in range(self.width):
                if (x, y) not in self.solved:
                    if not self.singleguess((x, y), _BLK): #Contradiction found
                        return ((x, y), _EMP)
                    if not self.singleguess((x, y), _EMP): #Contradiction found
                        return ((x, y), _BLK)
        return None
    ...
    def dump(self, path):
        '''
        Dumps the solved squares to a file. THIS is how my display method
        should look like !
        '''
        f = open(path, 'w')
